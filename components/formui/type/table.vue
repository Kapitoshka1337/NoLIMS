<template>
    <v-dialog dense v-model="getVisible" @input="closeDialog()">
        <v-card>
            <v-card-title>Типы оборудования</v-card-title>
            <v-divider></v-divider>
            <v-card-text>
                <v-data-table
                single-select
                ...
                <template #top>
                    <v-toolbar color="white" flat>
                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                                <v-btn v-bind="attrs" v-on="on" icon @click="showDialogCreate = true" disabled><v-icon>mdi-plus</v-icon></v-btn>
                            </template>
    ...
    async getData() {
        try
        {
            ...
            if (this.options.sortBy.length <= 0)
                url = `api/v1/type?pageNumber=${this.options.page}&pageSize=${this.options.itemsPerPage}`;
            else
                url = `api/v1/type?pageNumber=${this.options.page}&pageSize=${this.options.itemsPerPage}&sortBy=${this.options.sortBy[0]} ${this.options.sortDesc[0] ? "desc" : ""}`;
            ...
            this.$toast.success("Типы оборудования успешно загружены.");
            this.load = false
        }
        catch (e)
        {
            this.$toast.error("Ошибка во время загрузки типов оборудования.");
            this.load = false
        }
    }
