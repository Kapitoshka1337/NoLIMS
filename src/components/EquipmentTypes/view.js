...
class EquipmentTypeView extends React.PureComponent {
    ...
    async getData(page, size, sorter){
        ...
        {
            const data = await agent.EquipmentTypeService.view(this.state.currentPage, this.state.pageSize, this.state.sorter)
            this.setState({...this.state, dataSource: data, loading: false, sorter: sorter})
        }
        else
        {
            const data = await agent.EquipmentTypeService.view(page, size, sorter)
    ...
    openCard(record){
        history.push(`/equipment/types/view/${record.id}`)
    }
