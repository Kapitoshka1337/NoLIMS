...
export default {
	...
	methods: {
		getInstructions(){
			this.load = true;
			this.$http.get('/api/equipment/instructions').then(response => (this.load = false, this.gridData = response.data)).catch(error => (this.load = false, alert(error.response.data.message)));
		},
		editedItem(item){
		...
		submit(){
			...
			else
			{
				...
				formData.append('file', this.item.file);
				this.loading = true;
				this.$http.post("/api/equipment/instructions", formData, {headers: {'Content-Type': 'multipart/form-data'}})
				.then(response => (this.loading = false, this.dialog = false, this.getInstructions())).catch(error => (this.loading = false, alert(error.response.data.message)));
			}
		},
        download(item){
            this.overlay = true;
