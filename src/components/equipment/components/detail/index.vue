		...
		<v-col cols="12">
			<v-card outlined>
				<v-card-title>
					...
					<v-spacer></v-spacer>
					<v-dialog v-model="dialogMaintenance" max-width="712px" v-if="getAllowSave">
						<template v-slot:activator="{ on, attrs }">
							<v-btn icon color="orange" v-bind="attrs" v-on="on">
								<v-icon>mdi-plus</v-icon>
							</v-btn>
						...
						<v-card>
							...
							<v-card-text>
								...
								<v-row>
									<v-col cols="12">
										...
										<v-text-field v-model="main.periodicity" type="number" clearable outlined dense label="Периодичность (указывать в днях)"></v-text-field>
									</v-col>
								</v-row>
							</v-card-text>
							<v-divider></v-divider>
							<v-card-actions>
								<v-spacer></v-spacer>
								<v-btn color="success" :loading="loadMoving" @click="submitMaintenance()">Добавить ТО</v-btn>
								<v-btn color="error" @click="dialogMaintenance = false">Не добавлять</v-btn>
							</v-card-actions>
						</v-card>
					</v-dialog>
				</v-card-title>
...
export default {
	...
	methods: {
		...
		submitMaintenance(){
			...
			this.loadMoving = true;
			this.$http.post('/api/equipment/equipments/services', this.main, {headers: {'Content-Type': 'application/json'}})
