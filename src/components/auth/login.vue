...
export default {
	...
	methods: {
		getUsers(){
			...
			this.$http.get(this.env + 'structure/users').then(response => (this.users = response.data, this.loading = !this.loading)).catch(error => (alert(error.response.data.message), this.loading = !this.loading));
		},
		login(){
			this.loading = !this.loading;
