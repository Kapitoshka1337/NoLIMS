<template>
	<div class="padded" is="sui-grid">
		<sui-grid-row>
			<sui-grid-column>
				<sui-menu :width="3">
					<router-link to="#" is="sui-menu-item">Поступления</router-link>
					<router-link to="#" is="sui-menu-item">Потребление</router-link>
				...
				<sui-table selectable compact v-if="gridData.length > 0">
					...
					<sui-table-body>
						<sui-table-row v-for="(material, index) in paginateRows" :key="index">
							...
							<sui-table-cell collapsing>
								<button class="ui icon mini blue button" v-if="material.total <= 0 || colorShelfLife(material.shelf_life) <= 1" v-on:click="moveToArchive(material.arrival_material_id)"><i class="icon archive"></i></button>
...
export default {
	...
	methods: {
		...
		//},
		moveToArchive(id){
			axios.put("/api/reagent/storage/archive", JSON.stringify({id: id}), {
					headers: {'Content-Type': 'application/json'}}).then( response => (alert(response.data)));
		},
	},
	watch: {
		gridData(){
