...
class InstructionView extends React.PureComponent {
    ...
    getData = async (page, size, sorter) => {
        ...
        {
            const data = await agent.InstructionService.view(this.state.currentPage, this.state.pageSize, this.state.sorter, this.state.filters)
            this.setState({...this.state, dataSource: data, loading: false, sorter: sorter})
        }
        else
        {
            const data = await agent.InstructionService.view(page, size, sorter, this.state.filters)
