export class EmployesService {
    ...
    computedUrl(options)
    {
        ...
        if (options.sortBy.length <= 0)
            url = `api/v1/user?pageNumber=${options.page}&pageSize=${options.itemsPerPage}`;
        else
          url = `api/v1/user?pageNumber=${options.page}&pageSize=${options.itemsPerPage}&sortBy=${options.sortBy[0]} ${options.sortDesc[0] ? "desc" : ""}`;
        
        return url
    }

    async view (options, filter) {
      if ($nuxt.$permissions.can('view', 'user'))
      {
        let data;
      ...
      {
        this.$toast.error("У вас нет прав на просмотр сотрудников.");
      }
    }
  }